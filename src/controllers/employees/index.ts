// backend/src/controllers/employees/index.ts
...
class Employees {
   ...
   */
  private combineDescriptors(descriptors: number[][]): number[] {
   ...
   */
  public async recognize(req: MulterRequest & { body?: Record<string, any> }, res: Response): Promise<void> {
    try {
      const body = req.body || {}
      ...
      const profiles = await FaceProfile.find({}).lean().exec()
      ...
      let best: { profile: any | null; distance: number; employee: any | null } = { 
        profile: null, 
      ...
      for (const p of profiles) {
        if (!p.facial_embeddings || !Array.isArray(p.facial_embeddings)) continue
        
        try {
          const dist = this.euclideanDistance(descriptor, p.facial_embeddings as number[])
      ...
      if (best.profile && best.distance <= THRESHOLD) {
        ...
        const employee = await EmployeesModel.findById(best.profile.employee_id).lean().exec()
